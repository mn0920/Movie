     ...
     <div class="main2">
      ...
      <div class="row2">
        <c:forEach var="director" items="${director}">
          <a href="<%= request.getContextPath() %>/admin/director/modify?director_id=${director.director_id}" class="none">
            <div class="card">
              <div class="content"> 
                <div class="poster">
                  <c:choose>
                ...
                <div class="name"><c:set var="TextValue" value="${director.director_name}"/>${fn:replace(director.director_name, '(', '<br>(')}</div>
                <p class="birth">${director.director_birth}</p>
                <p class="pro">${director.director_gender}</p>
                <a href="<%= request.getContextPath() %>/admin/director/modify?director_id=${director.director_id}"><button type="button" class="btn modify float-right">수정</button></a>
                <a href="<%= request.getContextPath() %>/admin/director/delete?director_id=${director.director_id}"><button type="button" class="btn delete float-right">삭제</button></a>
              </div>
            </div>
          </a>
        </c:forEach>
